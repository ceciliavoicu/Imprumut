    ...
    <v-responsive>
      <v-form>
        <v-card>
          ...
          <v-select :items="cititorImport" v-model="imprumut.cititor" label="Cititor" item-title="nume" item-value="id"
                    return-object></v-select>
          <v-select :items="carteImport" v-model="imprumut.carte" label="Carte" item-title="nume" item-value="id"
                    return-object></v-select>
          <Datepicker v-model="oldDateImprumut" :teleport="true" :enable-time-picker="false" auto-apply
                      @update:model-value="changeDateImprumut">
            <template #trigger>
              <v-text-field v-model="imprumut.dataImprumut" label="Data imprumutarii" readonly
                            @click="showCalendarImprumut=true">
              </v-text-field>
            </template>
          </Datepicker>
          <Datepicker v-model="oldDateReturn" :teleport="true" :enable-time-picker="false" auto-apply
                      @update:model-value="changeDateReturnare">
            <template #trigger>
              <v-text-field v-model="imprumut.dataReturnarii" label="Data returnarii" readonly
                            @click="showCalendarReturn=true">
              </v-text-field>
            </template>
          </Datepicker>
